...
import { addDoc, doc, setDoc, getDocs, query, collection, where, updateDoc } from 'firebase/firestore';
...
export default ({ state, setState, actions }: any) => ({
    ...
    getAuthUser: async () =>{
        try {
            ...
            setState((state: any) => ({ ...state, users: { ...state.users, logged: true, user: data.user } }));
        } catch {
        }
    },
    ...
    signUp: async (newUser: any, history: any) => {
        try {
            await createUserWithEmailAndPassword(auth, newUser.email, newUser.password).then(async (userCredential: any) => {
    ...
    logIn: async( email: string, password: string ) => {
        try {
            await signInWithEmailAndPassword(auth, email, password).then(async (userCredential: any) => {
                ...
                const dataQuery = query(collection(db, "users"), where("role", "==", "OWNER"));
    ...
    logInPlayers: async( user: any ) => {
        const setLocalUserState = () => {
            ...
            setState((state: any) => ({ ...state, users: { ...state.users, logged: true, user: user } }));
        }
        try {
    ...
    updateUser: async( user: any ) => setState((state: any) => ({ ...state, users: { user } })),
